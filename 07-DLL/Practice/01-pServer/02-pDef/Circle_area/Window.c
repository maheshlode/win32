...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg)
	{
        case WM_CREATE:
            ...
            pfnMakeCube=(MakeCubeFunctionPointer)GetProcAddress(hDLL,"MakeCircleArea");
            if(pfnMakeCube==NULL)
            {
            ...
            iCircle=pfnMakeCube(iRadius);
            wsprintf(str,TEXT("Area od circle with radius %d is %d "),iRadius,iCircle);
            MessageBox(hwnd,str,TEXT("Area od circle Calculation"),MB_OK);
            FreeLibrary(hDLL);
            hDLL=NULL;
            DestroyWindow(hwnd);
            break;
